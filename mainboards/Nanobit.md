...
锂电池供电PH2.0接口：3.7-4.2V（nanobit板子上不含锂电池充电电路，因此不能给锂电池充电）
...
主板与编程下载器采用可掰断设计，降低用户制作成本。（新手入门暂不建议掰断，因为掰断需要自己进行焊接端子）
...
这里接了0.96寸的OLED（128x64像素），因此直接选择128x64（0.91寸的oled对应是128x32）
