...
鸢尾花分类相当于机器学习中的Helloworld问题，如果这个问题你能解开了那么说明你机器学习已经入门了。

大家平时应该也留意过这种花吧，但是真的能分出来三种花的区别吗？

![](./images/c3_01.jpg)
![](./images/c3_02.jpg)
![](./images/c3_03.jpg)

这篇教程所使用的的数据全称是：安德森鸢尾花卉数据集，完整的数据如下表：
...
我们将上面的数据保存到硬盘上，名字叫iris.csv。之后在Kittenblock内新建一个项目，导入TensorFlow插件。

第一步首先我们要建立两个列表（list）变量，分别叫xs和ys，对应我们的前四列输入数据和最后一列输出名字。
...
例如：`5.1,3.5,1.4,0.3,Iris-setosa`这一组数据的输出是**Iris-setosa**，但是机器并不认识这个但是。它跟喜欢的是形如**[1,0,0]**这样的矩阵，这样它就知道这一组数据输出是第一个类别。

同理：`6.2,2.9,4.3,1.3,Iris-versicolor`这一组的数据输出就应该是**[0,0,1]**

因此我们这里需要使用Output Label方块，这个方块将ys数据直接转换成onehot矩阵，并且保存了对应的标签映射。
...
大家可以打开Kittenblock的调试窗口查看模型的收敛过程：
...
注意我们将ys标签化了，所以我们需要用Predict By Label让它输出正确的方块。 电机方块猫咪就会告诉你正确的类别喽~

![](./images/c3_12.png)

当然大家也可以不用标签输出，那么模型预测将给出三组概率，其中最高那个就是我们需要的类别了。
