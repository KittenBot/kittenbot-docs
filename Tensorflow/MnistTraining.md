...
拖入如下的方块，并点击。舞台背景转换为矢量图，再填充黑色，默认的白色背景，看不请数字图片
...
上面的显示图片的小程序，可以删除或者忽略。我们现在照着下面的图片拖入对应的模型：
...
强烈建议按键键盘ESC下方的“~按键” 打开后台调试窗口，因为这个计算量很巨大，不会马上返回训练完成信息，我们往往会误认为电脑死机或者积木块没有执行。
